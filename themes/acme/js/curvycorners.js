...
/* Unus */
  ...
  var newMainContainer = document.createElement("div");
	newMainContainer.id=this.box.id+"_newMainContainer";  
	newMainContainer.className="cornersContainer";
  ...
  // Kinetic Data - the style "cornerResizeNeeded" must be applied to a container that will be expanded/contracted and has borders
  // Be careful not to apply this className to an embedded situation.
  if (this.box.className.indexOf("cornerResizeNeeded") != -1) {
  	newMainContainer.style.setExpression("height", "parentNode.clientHeight-8+'px'");
  } else {
...
// Unus    this.contentContainer.style.position = "absolute";
...
/* KD HACK  - set width to 0 if it is calculated as a negative value */
//  return (this.boxWidth - this.spec.radiusSum(tb) + bWidth) + 'px';
  var width = (this.boxWidth - this.spec.radiusSum(tb) + bWidth);
  if (width < 0) {
    width = 0;
  }
  return width + 'px';
